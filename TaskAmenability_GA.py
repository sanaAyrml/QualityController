...
class TaskAmenability_GA(gym.Env):
    
    def __init__(self, x_train, x_val, task_predictors,device,log_dir,tensorboard=None):
        ...
        self.task_predictors = task_predictors
    ...
    def get_val_acc_vec(self,num):
        self.task_predictors[num].eval()
        with torch.no_grad():
            correct = []
            # total = 0
            for images, labels,if_noisy in self.x_val_loader:
                ...
                outputs = self.task_predictors[num](images)
                _, predicted = torch.max(outputs.data, 1)
                correct += (predicted == labels).cpu()
        return np.array(correct)
    
    def train_predictor(self,train_data, num,mode = 'PPO'):
        train_loader = torch.utils.data.DataLoader(dataset=train_data,
                                                   batch_size=self.task_predictor_batch_size, shuffle=True)
        self.learning_rate = 0.001
        self.optimizer = torch.optim.Adam(self.task_predictors[num].parameters(), lr=self.learning_rate)
        curr_lr = self.learning_rate
        self.task_predictors[num].train()
    ...
    def step(self, action,num):
        ...
        if self.sample_num_count < self.controller_batch_size:
            ...
            if len(x_train_selected) < 1:
                ...
                self.train_predictor(x_train_selected,num,'PPO')
                
                val_acc_vec = self.get_val_acc_vec(num)
...
#                 print(val_sel_vec_normalised.shape)
                ...
                if val_metric > self.best_valid_metric:
                    ...
                    torch.save(self.task_predictors[num].state_dict(), self.saving_path)
    ...
    def save_task_predictor(self, task_predictor_save_path):
        self.task_predictors[num].save(task_predictor_save_path)
